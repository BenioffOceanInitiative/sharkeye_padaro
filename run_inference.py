...
def save_detected_shark_frame(frame, frame_no_bb, frame_number, track_id, avg_conf, max_conf, min_conf, length, video_fps, video, csv_writer, survey_path):
    grade_folder = get_grade_folder(avg_conf)
    bb_save_path = os.path.join(survey_path, 'bb', grade_folder)
    no_bb_save_path = os.path.join(survey_path, 'no_bb', grade_folder)
...
def run_inference(video_directory='survey_video', model_weights_path='model_weights/exp1v8sbest.pt', altitude=30, show_ui=False, years=None):
    ...
    survey_path = os.path.join(results_path, f'survey_{survey_datetime}')
    os.makedirs(survey_path, exist_ok=True)
    
    with open(os.path.join(survey_path, f'shark_detections_{survey_datetime}.csv'), mode='w', newline='') as csv_file:
